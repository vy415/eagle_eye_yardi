...
class eagle_yardi:
	...
	def __init__(self, directory, output, args):
		...
		self.header_fields_raw = ["Property ID", "Property_Name", "Name", "Source", "Event Date", "Action"]
		self.header_fields = ["Property ID", "Property_Name", "Name", "First Name", "Last Name", "Source", "Event Date", "Action"]
		self.excel_files = []
		self.keys = {}
				...
				#Begin collecting names after getting past the header row
					...
					if name_cell != "":
						...
						if self.args.raw == True:
							person = {'Property ID': self.keys.get(prop_key, ''), 'Property_Name': prop_name_cell, 'Name': name_cell, 'Source': source_cell, 'Event Date': event_date_cell,'Action': section_title_cell}
						else:
							person = {'Property ID': self.keys.get(prop_key, ''), 'Property_Name': prop_name_cell, 'Name': name_cell, 'Source': source_cell, 'Event Date': event_date_cell,'Action': section_title_cell}
...
def main():
	...
	parser.add_argument('-k', '--keys', nargs='?', type=str, help='Enter the property ID keys file.')
