			...
			for excel_file in self.excel_files:
				...
				for row_index in range(worksheet.nrows):
					...
					if 'property:' in cell.lower():
						...
						start = True
					elif start and prop_name_flag != True:
						if cell != "" and all(i == "" for i in row[1:]) and cell.replace(" ", "") not in [i.replace(" ", "") for i in self.section_titles]:
							prop_name_cell = worksheet.cell(row_index, 0).value
							writer.writerow([prop_name_cell])
